...
Soil type typically reflects commons soil properties, vertical soil stratification / 
soil horizons, diagnostic features, sometimes also soil-forming factors such as 
climate and hydrological conditions. Unlike biology and species taxonomy, soil types are abstract and 
...
hence they can be considered fuzzy geographical features [@lodwick2007fuzzy].
...
so that even though different countries use somewhat different concepts, many soil classification systems are really comparable [@micheli2016testing]. 
Correlation is, nevertheless, not always ideal and it is good to consider that one soil type in a local 
system could be translated to at least 2 or 3 soil types in the international (target) system.

In this computational notebook we show how to generate a global map of soil types, how to access 
...
so if you see an artifact or an issue, [please report](https://github.com/OpenGeoHub/SoilTypeMapping/issues).
...
from the HWSDv2. This gave us about [70,000 training points](https://opengeohub.github.io/SoilSamples/wrb-soil-types.html) in total (see figure below). 
...
using the terra package [@hijmans2019spatial], which can also be run in parallel by using e.g.:
...
please report via the [Github repository](https://github.com/OpenGeoHub/SoilTypeMapping/issues).
